...
$(function(){if('createCode'== locat[3]){locat =  locat[0]+'//'+locat[2];}else{locat =  locat[0]+'//'+locat[2]+'/'+locat[3];};});
		...
		if("" != CREATECODE_ID){
			...
			$.ajax({
				...
				success: function(data){
					...
					var tb = data.pd.TABLENAME.split(",llft,");
					$("#tabletop").val(tb[0]);
	...
	function saveD(){
		...
		var fields = dname + ',llft,' + dtype + ',llft,' + dbz + ',llft,' + isQian + ',llft,' + ddefault + ',llft,' + flength + ',llft,' + decimal;
	...
	function editField(value,msgIndex){
		$("#dialog-add").css("display","block");
		var efieldarray = value.split(',llft,');
	...
	function appendC(value){
		var fieldarray = value.split(',llft,');
		$("#fields").append(
			'<tr>'+
	...
	function isSame(value){
		for(var i=0;i<arField.length;i++){
			var array0 = arField[i].split(',llft,')[0];
			if(array0 == value){
				return false;
			}
		}
