				...
				if(result) {
					...
					for(var i=0;i < document.getElementsByName('ids').length;i++){
					  if(document.getElementsByName('ids')[i].checked){
					  	...
					  	else str += ',llft,' + document.getElementsByName('ids')[i].value;
					  	
					  	if(fid=='') fid += document.getElementsByName('ids')[i].id;
					  	else fid += ',llft,' + document.getElementsByName('ids')[i].id;
					  }
					}
					if(str==''){
						bootbox.dialog({
						...
						if(msg == '确定要批量备份这些表吗？'){
							var arrTable = str.split(',llft,');
							var arrFid = fid.split(',llft,');
