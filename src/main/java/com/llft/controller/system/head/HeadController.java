package com.llft.controller.system.head;
...
import com.llft.service.system.appuser.AppuserManager;
import com.llft.service.system.fhsms.FhsmsManager;
import com.llft.service.system.user.UserManager;
import com.llft.service.system.userphoto.UserPhotoManager;
import com.llft.util.AppUtil;
import com.llft.util.Const;
import com.llft.util.Jurisdiction;
import com.llft.util.PageData;
import com.llft.util.SmsUtil;
import com.llft.util.Tools;
import com.llft.util.Watermark;
import com.llft.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",llft,"+pd.getString("PORT")+",llft,"+pd.getString("EMAIL")+",llft,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",llft,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",llft,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",llft,"+pd.getString("fcontent")+",llft,"+pd.getString("fontSize")+",llft,"+pd.getString("fontX")+",llft,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",llft,"+pd.getString("imgUrl")+",llft,"+pd.getString("imgX")+",llft,"+pd.getString("imgY"));	//图片水印配置
		Watermark.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	...
	public ModelAndView saveSys3() throws Exception{
		...
		Tools.writeFile(Const.WEBSOCKET,pd.getString("WIMIP")+",llft,"+pd.getString("WIMPORT")+",llft,"+pd.getString("OLIP")+",llft,"+pd.getString("OLPORT")+",llft,"+pd.getString("FHsmsSound"));	//websocket配置
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public ModelAndView saveSys4() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.LOGINEDIT,pd.getString("isZhuce")+",llft,"+pd.getString("isMusic"));	//登录页面配置
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
